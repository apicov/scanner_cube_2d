...
def run_test_rnd(models_path,model,n_images):
    ...
    for i in range(n_images):
        theta = np.random.randint(TOTAL_THETA)
        phi = np.random.randint(TOTAL_PHI)
        spc.carve( theta, phi )
    return  spc.gt_compare_solid()


def test_agent_rnd_policy(models_path, model, n_images, init_position, theta_bias):
...
f = open("uni_rnd_policy_runs_352_v3.json",'w')
...
for m in models:
    ...
    for i in range(n_init_positions):
        gt_dist = run_test_rnd(models_path,m,n_images)
        m_data['rnd']['gt_dists'].append(gt_dist)
        print("\r{} {}                  ".format(i,gt_dist),end="")
    ...
    #collect data random policy in evironment
    for i in theta_phi_posbias:
        time1 = time.time()
        cum_reward, gt_dist = \
            test_agent_rnd_policy(models_path, m, n_images, init_position=i[:2], theta_bias=i[2])

        m_data['agent_rnd']['cum_reward'].append(cum_reward)
        m_data['agent_rnd']['gt_dists'].append(gt_dist)
       
        time2 = time.time()
        print("\r{} {} {} {}                        ".format(i, m_data['agent_rnd']['cum_reward'][-1], m_data['agent_rnd']['gt_dists'][-1],time2-time1),end="")


    m_data['agent_rnd']['cum_reward_mean'] = float(np.mean(m_data['agent_rnd']['cum_reward']))
    m_data['agent_rnd']['cum_reward_std'] = float(np.std(m_data['agent_rnd']['cum_reward']))
    m_data['agent_rnd']['gt_mean'] = float(np.mean(m_data['agent_rnd']['gt_dists']))
    m_data['agent_rnd']['gt_std'] = float(np.std(m_data['agent_rnd']['gt_dists']))

    print('cum_reward_mean',m_data['agent_rnd']['cum_reward_mean'],
          'cum_reward_std',m_data['agent_rnd']['cum_reward_std'])
    print('gt_mean',m_data['agent_rnd']['gt_mean'],'gt_std',m_data['agent_rnd']['gt_std'])
    ...
    for i in range(steps):
        ...
        time2 = time.time()
        print("\r{} {} {} ".format(i, m_data['uni']['gt_dists'][-1],time2-time1), end="")
