...
class ScannerEnv(gym.Env):
    ...
    def __init__(self,models_path,train_models,n_images = 10, continuous=False,gt_mode=True,cube_view='static'):
        ...
        # theta , phi  positions and number of images collected
        lowl = np.array([0,0,1])
        highl = np.array([self.theta_n_positions-1, self.phi_n_positions-1, 20])
        self.vec_ob_space = gym.spaces.Box(lowl, highl, dtype=np.int32)
    ...
    def reset(self,theta_init=-1,phi_init=-1,theta_bias=-1):
        ...
        self.current_state = ( vol.astype('float16'), np.array([self.current_theta, self.current_phi, self.num_steps+1],dtype=int))
    ...
    def step(self, action):
        ...
        if self.gt_mode is True:
            ...
            reward = delta_gt_ratio * self.num_steps
